...
while True:
    try:
        ...
        invalid_month = (
            False
            if date.find("/") == -1
            else (True if date[date.find("/") - 1].isnumeric() == False else False)
        )
        if invalid_month:
            continue
        ...
        if valid_month(month) and valid_day(day):
            ...
            if len(month) > 3:
                month = (
                    "0" + str(months_list.index(month) + 1)
                    if len(str(months_list.index(month) + 1)) == 1
                    else months_list.index(month) + 1
                )
